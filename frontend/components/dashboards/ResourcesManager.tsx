...
export default function ResourcesManager() {
  ...
  const fetchList = async () => {
    ...
    setError(null);
    try {
      const result = await getResources({ page, limit, search, category });
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (editingId) {
      ...
      if (!previous) return;
      const optimistic = { ...previous, ...form } as ResourceItem;
      setItems((prev) => prev.map((item) => (item.id === editingId ? optimistic : item)));
      resetForm();
      try {
        const updated = await updateResource(editingId, form);
    ...
    const optimistic = { id: tempId, ...form } as ResourceItem;
  ...
  return (
    <div className="grid gap-8">
      <div className="card">
        <h2 className="text-xl font-semibold">{editingId ? "Edit Resource" : "Create Resource"}</h2>
        <form onSubmit={handleSubmit} className="mt-6 grid gap-4 md:grid-cols-2">
          ...
          <input
            className="input"
            placeholder="Category (DSA, Web, CP)"
            value={form.category}
      ...
      <div className="card">
        ...
        {loading ? <p className="mt-6 text-sm text-slate-400">Loading resources...</p> : null}
        {!loading && items.length === 0 ? (
          <p className="mt-6 text-sm text-slate-400">No resources found.</p>
        ) : null}
